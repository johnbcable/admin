...
If searchwhere <> "" Then
	Session("membersemail_searchwhere") = searchwhere

	' Reset start record counter (new search)
	startRec = 1
	Session("membersemail_REC") = startRec
Else
	searchwhere = Session("membersemail_searchwhere")
End If
%>
<%
...
If Request.QueryString("order").Count > 0 Then
	...
	If Session("membersemail_OB") = OrderBy Then
		If Session("membersemail_OT") = "ASC" Then
			Session("membersemail_OT") = "DESC"
		Else
			Session("membersemail_OT") = "ASC"
		End if
	Else
		Session("membersemail_OT") = "ASC"
	End If
	Session("membersemail_OB") = OrderBy
	Session("membersemail_REC") = 1
Else
	OrderBy = Session("membersemail_OB")
	If OrderBy = "" Then
		OrderBy = DefaultOrder
		Session("membersemail_OB") = OrderBy
		Session("membersemail_OT") = DefaultOrderType
	End If
End If
...
' Build SQL
strsql = "SELECT * FROM [membersemail]"
...
End If
If OrderBy <> "" Then 
	strsql = strsql & " ORDER BY [" & OrderBy & "] " & Session("membersemail_OT")
End If	
...
ElseIf Request.QueryString("pageno").Count > 0 Then
	...
	If IsNumeric(pageno) Then
		...
		Session("membersemail_REC") = startRec
	Else
		startRec = Session("membersemail_REC")
...
<p><span class="aspmaker">VIEW: membersemail</span></p>
<form action="membersemaillist.asp">
<table border="0" cellspacing="0" cellpadding="4">
	<tr>
		...
		&nbsp;&nbsp;<a href="membersemaillist.asp?cmd=reset">Show all</a>
		</span></td>
	</tr>
...
<a href="membersemaillist.asp?order=<%= Server.URLEncode("surname") %>" style="color: #FFFFFF;">surname&nbsp;(*)<% If OrderBy = "surname" Then %><span class="ewTableOrderIndicator"><% If Session("membersemail_OT") = "ASC" Then %>5<% ElseIf Session("membersemail_OT") = "DESC" Then %>6<% End If %></span><% End If %></a>
		</span></td>
		<td><span class="aspmaker" style="color: #FFFFFF;">
<a href="membersemaillist.asp?order=<%= Server.URLEncode("forename1") %>" style="color: #FFFFFF;">forename 1&nbsp;(*)<% If OrderBy = "forename1" Then %><span class="ewTableOrderIndicator"><% If Session("membersemail_OT") = "ASC" Then %>5<% ElseIf Session("membersemail_OT") = "DESC" Then %>6<% End If %></span><% End If %></a>
		</span></td>
		<td><span class="aspmaker" style="color: #FFFFFF;">
<a href="membersemaillist.asp?order=<%= Server.URLEncode("email") %>" style="color: #FFFFFF;">email&nbsp;(*)<% If OrderBy = "email" Then %><span class="ewTableOrderIndicator"><% If Session("membersemail_OT") = "ASC" Then %>5<% ElseIf Session("membersemail_OT") = "DESC" Then %>6<% End If %></span><% End If %></a>
		</span></td>
</tr>
<%
...
<!--first page button-->
	...
	<% Else %>
	<td><a href="membersemaillist.asp?start=1"><img src="images/first.gif" alt="First" width="20" height="15" border="0"></a></td>
	<% End If %>
<!--previous page button-->
	<% If CLng(PrevStart) = CLng(startRec) Then %>
	<td><img src="images/prevdisab.gif" alt="Previous" width="20" height="15" border="0"></td>
	<% Else %>
	<td><a href="membersemaillist.asp?start=<%=PrevStart%>"><img src="images/prev.gif" alt="Previous" width="20" height="15" border="0"></a></td>
	<% End If %>
<!--current page number-->
...
<!--next page button-->
	...
	<% Else %>
	<td><a href="membersemaillist.asp?start=<%=NextStart%>"><img src="images/next.gif" alt="Next" width="20" height="15" border="0"></a></td>
	<% End If %>
<!--last page button-->
	<% If CLng(LastStart) = CLng(startRec) Then %>
	<td><img src="images/lastdisab.gif" alt="Last" width="20" height="15" border="0"></td>
	<% Else %>
	<td><a href="membersemaillist.asp?start=<%=LastStart%>"><img src="images/last.gif" alt="Last" width="20" height="15" border="0"></a></td>
	<% End If %>
